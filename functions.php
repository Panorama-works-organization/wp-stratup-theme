...
if ( ! function_exists( 'panorama_setup' ) ) :
	...
	function panorama_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'panorama' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'panorama_custom_background_args',
				array(
...
function panorama_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'panorama_content_width', 640 );
}
add_action( 'after_setup_theme', 'panorama_content_width', 0 );
...
function panorama_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'panorama' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'panorama' ),
...
function panorama_scripts() {
	wp_enqueue_style( 'panorama-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'panorama-style', 'rtl', 'replace' );

	wp_enqueue_script( 'panorama-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
